...
def check_compile(result_path, framework):
    """Check if generated code compiled without errors.
    Uses the same method as DesignBench's compile.py:
    - React: check saved .html for Next.js error overlay
    - Vue: check saved .html for Vue error message
    - Angular: check saved .log for angular-compiler errors
    - Vanilla: always True (no compilation step)
    ...
    ext = ".log" if framework == "angular" else ".html"
    # Result file is like .jsx / .vue / .angular — find the corresponding .html or .log
    base = os.path.splitext(result_path)[0]
    check_path = base + ext

    if not os.path.exists(check_path):
        # No rendered output at all = compile failure
        return False
    ...
    if framework == "angular":
        # Angular: check for compiler errors in log
        pattern = r".*\[plugin angular-compiler\].*"
        if re.search(pattern, content):
            return False
        # Also check if .html was produced (angular writes .html only on success)
        html_path = base + ".html"
        return os.path.exists(html_path)

    elif framework == "react":
        # React: check for Next.js error overlay in saved HTML
        if 'nextjs-container-errors' in content:
            return False
        # Legacy pattern for older Next.js versions
        pattern = r'style=color:var\(--color-ansi-truecolor\)>\×</span><span>'
        if re.search(pattern, content):
            return False

    elif framework == "vue":
        # Vue: check for Vue error message in saved HTML
        pattern = r'</span><span class=message-body part=message-body>(.*?)</span>'
        if re.search(pattern, content, re.DOTALL):
            return False

    # Also check for blank white page (rendered but empty)
    png_path = base + ".png"
