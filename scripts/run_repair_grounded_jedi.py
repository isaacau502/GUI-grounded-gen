"""Run DesignBench repair with JEDI click-point grounding injected into the prompt.

Wraps `ui-repair-baseline/run_repair.py`. Before delegating to the baseline
runner, monkeypatches:

  1. `prompt.repair_prompt.get_design_repair_prompt` — appends a natural-
     language list of (issue, click_point) pairs for the current sample,
     produced by `scripts/build_jedi_cache.py`.
  2. `runner.main.Runner.run_repair` — sets a thread-local (framework,
     ...
     which grounding entry to read.

Tags model_filename with `+jedi` so results don't collide with baseline or
the OmniParser variant.

Usage:
    # Pre-req: JEDI cache built on Colab, rclone'd to:
    #   jedi_cache.json in repo root (or set GROUNDING_CACHE env var).

    python scripts/run_repair_grounded_jedi.py --samples 2
    python scripts/run_repair_grounded_jedi.py --samples 2 --frameworks react
    python scripts/run_repair_grounded_jedi.py --full
"""
...
CACHE_PATH = Path(os.environ.get("GROUNDING_CACHE", REPO / "jedi_cache.json"))
if not CACHE_PATH.exists():
    sys.exit(
        f"JEDI grounding cache missing: {CACHE_PATH}. "
...
print(f"Loaded JEDI grounding for {len(GROUNDING_CACHE)} samples from {CACHE_PATH}.")
...
def _format_grounding_block(entry: dict) -> str:
...
def _get_grounding() -> str:
    ...
    if "error" in entry and not entry.get("issues"):
        return ""
    return _format_grounding_block(entry)
...
def _patched_init(self, model_name, framework, stream=True, print_content=False, *a, **kw):
    ...
    # Tag the filename so grounded runs don't collide with baseline or omni results.
    self.model_filename = f"{self.model_filename}+jedi"
...
def main():
    ...
    for fw_name in args.frameworks:
        ...
        print(f"\n{'='*60}\nGROUNDED-JEDI {args.model} on {fw_name} (samples {rng[0]}-{rng[1]-1})\n{'='*60}")
    ...
    tagged_model = f"{args.model}+jedi"
    ...
    print(f"\nJEDI-grounded run complete. Compare:")
